<template>
    <div class="tag-cloud" ref="wrapper">
        ...
        @dblclick="dbclickTag(item)" @mouseover="activeStyle(index)" @mouseout="leaveStyle(index)">{{item.name}}</p>
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    activeStyle(index){
      ...
      this.$refs.tag[index].style.color ='rgb(' + 0 + ',' + 204 + ',' + 255 + ')'
      this.$refs.tag[index].style.border = '1px dashed gray'
      this.$refs.tag[index].style.fontSize = '30px'
      this.$refs.tag[index].style.opacity = 1
      clearInterval(this.timer);
    },
    leaveStyle(index){
      this.activeIndex = -1
      this.$refs.tag[index].style.color =
            'rgb(' +
            Math.round(255 * Math.random()) +
            ',' +
            Math.round(255 * Math.random()) +
            ...
            Math.round(255 * Math.random()) +
            ')'
            this.$refs.tag[index].style.border = 'none'
            this._initTags()
    }
  }
}
